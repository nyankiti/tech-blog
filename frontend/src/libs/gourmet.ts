...
export const getGourmetPost = async (slug: string): Promise<GourmetPost | null> => {
...
export const getSortedGourmetPosts = async (): Promise<GourmetPost[]> => {
  const posts = await getAllGourmetPosts();
  return posts.sort((a, b) => compareDesc(new Date(a.visitedAt), new Date(b.visitedAt)));
};

export const getRelatedGourmetPosts = async (post: GourmetPost): Promise<GourmetPost[]> => {
  const otherPosts = (await getSortedGourmetPosts()).filter((p) => p.slug !== post.slug);

  // locationTagsが一つでも一致する投稿を取得
  const locationMatchedPosts = otherPosts.filter(
    (p) => p.slug !== post.slug && p.locationTags.some((tag) => post.locationTags.includes(tag)),
  );
  // gourmetTagsが一つでも一致する投稿を取得
  const gourmetMatchedPosts = otherPosts.filter(
    (p) => p.slug !== post.slug && p.gourmetTags.some((tag) => post.gourmetTags.includes(tag)),
  );
...
export const getGourmetTags = async (): Promise<string[]> => {
  const posts = await getAllGourmetPosts();
  return Array.from(new Set(posts.flatMap((post) => post.gourmetTags))).filter((tag) => tag !== '');
};
